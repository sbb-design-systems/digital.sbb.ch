...
key: button-lean-variants-states_en
...
{{ imageOnGreyBackground({
  ...
  alt: 'Image of an alternative primary button',
...
{{ imageOnGreyBackground({
  ...
  alt: 'Image of a secondary button',
...
{{ imageOnGreyBackground({
  ...
  alt: 'Image of an icon button',
  title: 'Icon Button'
}) }}

When hovering over an icon button, a <sbb-link variant="inline" type="button" href="/{{page.lang}}/design-system/lean/components/tooltip">tooltip</sbb-link>  should be used to make the underlying action clearer.{.lead}

### Multiple actions
{{ imageOnGreyBackground({
  url: '/assets/images/lean/Button_More_Actions',
  alt: 'Image of a button with multiple actions',
  title: 'Button with multiple actions'
}) }}
