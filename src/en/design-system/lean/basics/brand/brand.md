---
layout: pattern.njk
tags: 
    - lean_en
    - lean_basics_en
    - page
key: brand-lean_en
title: Brand
parent: basics-lean_en
image: lean/overview/brand.webp
keywords: logo, brand, signet, pleitegeier
order: 10
---

## Use
* The logo is used in all SBB applications.{.lead}

## Rules
* The logo appears exactly once per page and is always located in the <sbb-link variant="inline" type="button" href="/{{page.lang}}/design-system/lean/components/header">header</sbb-link>.{.lead}
* It is always positioned top right.{.lead}
* Variations of the logo are not permitted.{.lead}
* The logo is not linked.{.lead}

## Variants
### Default
{{ imageOnGreyBackground({
  url: '/assets/images/lean/logo_default',
  alt: 'Image of the SBB logo',
  title: 'SBB logo'
}) }}

## Specifications
{{ specificationLinks({
  figma: 'https://www.figma.com/file/nAx0LVMACEr9tefiZRQkxv/Lean?node-id=173%3A138444&t=ZF5MfeXb0Gz2GkUw-1'
}) }}