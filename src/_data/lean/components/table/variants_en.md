...
* The individual columns can be filtered according to content type (text, date, value, etc.) using the appropriate components (<sbb-link  type="button" href="/en/design-system/lean/components/textfield/">Textfield</sbb-link>, <sbb-link  type="button" href="/en/design-system/lean/components/datepicker/">Datepicker</sbb-link>, <sbb-link  type="button" href="/en/design-system/lean/components/select/">Select</sbb-link>, etc.). {.lead}
* A separate <sbb-link  type="button" href="/en/design-system/lean/components/filter/">Filter</sbb-link> can also be used to restrict large amounts of data. {.lead}
...
* The actions are offered by using <sbb-link  type="button" href="/en/design-system/lean/components/button/">icon button</sbb-link>. Depending on the use case, it is decided in which column (first or last column) they are placed. There they overlay the existing content. {.lead}
* A maximum of 3 actions should be offered. If more are needed, a <sbb-link  type="button" href="/en/design-system/lean/components/contextmenu/">contextmenu</sbb-link> should be used. {.lead}
...
* All versions of <sbb-link  type="button" href="/en/design-system/lean/components/button/">buttons</sbb-link> can be used for the actions. {.lead}
