...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const tbody = document.getElementById("table-body");
    const resultCount = document.getElementById("result-count");
    wordlist_fr.sort((a, b) => a.Begriff.localeCompare(b.Begriff, "fr"));
    ...
    function renderTable(data) {
        ...
        resultCount.textContent = `${data.length} ${data.length === 1 ? "Entrée" : "Entrées"}`;
    }
    ...
    // --- Filtern ---
    function filterList(term) {
        if (!term) return wordlist_fr;
        const lower = term.toLowerCase();
        return wordlist_fr.filter(entry =>
            entry.Begriff.toLowerCase().includes(lower) ||
            entry.Alternative.toLowerCase().includes(lower) ||
            entry.Hinweis.toLowerCase().includes(lower)
        );
